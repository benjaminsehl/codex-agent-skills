...
- PR #15 merged (discovery brief + spec on `main`). Ran `plan` → `tasks/plan.md` + `tasks/todo.md` checklist: 5 dependency-ordered tasks (stack detection script → `assemble` skill + surface 13→14 → persistence → `init` integration → boundaries/smoke) with 2 checkpoints. Noted Stage 0 (PR #14) resolved the status-block dependency. Engineering choice: stack detection as `detect_stack.py`. Stopped at founder review gate; next is `build` T1.
